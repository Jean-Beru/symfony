...
class OidcTokenGenerator
{
    ...
    public function generate(string $userIdentifier, ?string $algorithmAlias = null, ?string $issuer = null, ?int $ttl = null, ?int $notBefore = null): string
    {
        ...
        $payload = [
            ...
            'iat' => $this->clock->now()->getTimestamp(), # https://datatracker.ietf.org/doc/html/rfc7519#section-4.1.6
        ...
        if ($ttl) {
            $payload['exp'] = $this->getExpires($ttl); # https://datatracker.ietf.org/doc/html/rfc7519#section-4.1.4
        }
        if ($notBefore) {
            $payload['nbf'] = $notBefore; # https://datatracker.ietf.org/doc/html/rfc7519#section-4.1.5
        }
    ...
    private function getExpires(int $ttl): int
    {
        if (0 > $ttl) {
            throw new \InvalidArgumentException('Time to live must be a positive integer.');
        }

        return $this->clock->now()->add(new \DateInterval("PT{$ttl}S"))->getTimestamp();
    }
}
